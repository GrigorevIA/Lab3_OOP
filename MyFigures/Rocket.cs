...
namespace MyFigures
{
    public class Rocket : Figure
    {
        ...
        public override void Draw()
        {
            ...
            {
                g.FillPolygon(new SolidBrush(Color.Gray), part.points);
                part.Draw(false);
            }
            g.FillPolygon(new SolidBrush(Color.Blue), Rocket_parts[1].points);
        ...
        public void DeleteF(Rocket figure, bool flag)
        {
            ...
            ShapeContainer.RocketsList.Remove(figure);
            ClearMap();
            foreach (Figure f in ShapeContainer.RocketsList)
            {
                f.Draw();
            }
            if (flag != true)
            {
                ShapeContainer.RocketsList.Add(figure);
            }
            pictureBox.Image = bitmap;
        }
    }
